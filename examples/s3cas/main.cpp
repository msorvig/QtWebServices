...
void transferDirectory(const QString sourcePath, const QString &targetPathPrefix = QString())
{
    ...
    QtS3 s3(environment.value("S3CAS_AWS_ACCESS_KEY_ID"),
            environment.value("S3CAS_AWS_SECRET_ACCESS_KEY"));
    if (s3.errorCode()) {
    ...
    auto transferFileContents =
        [&](const QString &targetFilePath, const QByteArray &fileContent, QStringList headers) {
        s3.clearErrorState();
